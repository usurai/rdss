...
auto RedisParser::Parse(Buffer str, uint32_t* consumed, RespExpr::Vec* res) -> Result {
    *consumed = 0;
    ...
    if (!cached_expr_) cached_expr_ = res;
    ...
    while (true) {
        ...
        for (; last_stashed_index_ < cur.size(); ++last_stashed_index_) {
            ...
            if (RespExpr::STRING == e.type) {
                ...
                if (ebuf.empty() && last_stashed_index_ + 1 == cur.size()) break;
        ...
        if (last_stashed_level_ + 1 == stash_.size()) break;
        ++last_stashed_level_;
        last_stashed_index_ = 0;
    }
}
...
auto RedisParser::ParseInline(Buffer str) -> Result {
    ...
    last_consumed_ = static_cast<uint32_t>(ptr - str.data());
...
auto RedisParser::ParseNum(Buffer str, int64_t* res) -> Result {
    ...
    last_consumed_ = static_cast<uint32_t>(pos - s) + 2;
...
auto RedisParser::ConsumeArrayLen(Buffer str) -> Result {
    ...
    if (server_mode_ && (parse_stack_.size() > 0 || !cached_expr_->empty())) return BAD_STRING;
...
auto RedisParser::ParseArg(Buffer str) -> Result {
    ...
    if (c == '$') {
        ...
        case BAD_INT:
            return BAD_ARRAYLEN;
        case OK:
            if (len < -1 || len > kMaxBulkLen) return BAD_ARRAYLEN;
            break;
        default:
            LOG(ERROR) << "Unexpected result " << res;
        }
        ...
        if (len < 0) { // Resp2 NIL
            ...
            DVLOG(1) << "String(" << len << ")";
            cached_expr_->emplace_back(RespExpr::STRING);
            bulk_len_ = static_cast<uint32_t>(len);
            state_ = BULK_STR_S;
        }
    ...
    if (c == '+' || c == '-') { // Simple string or error.
        ...
        if (eol[-1] != '\r' || !absl::SimpleAtoi(tok, &ival)) return BAD_INT;
        ...
        if (eol[-1] != '\r' || !absl::SimpleAtod(tok, &dval)) return BAD_INT;
    ...
    last_consumed_ = static_cast<uint32_t>(eol - s) + 2;
    state_ = FINISH_ARG_S;
    return OK;
}
...
auto RedisParser::ConsumeBulk(Buffer str) -> Result {
    ...
    if (str.size() >= 32) {
        ...
        last_consumed_ = static_cast<uint32_t>(len);
        bulk_len_ -= static_cast<uint32_t>(len);
    }
...
void RedisParser::HandleFinishArg() {
    ...
    while (true) {
        ...
        if (parse_stack_.back().first != 0) break;
